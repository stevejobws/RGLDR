...
# def attention4(hops, adj, feature_list, alpha=0.15):
    # input_feature = []
    ...
    # for i in range(100):
        # with tf.GradientTape() as tape:
            # pre = att(input_feature.detach().numpy())
            # loss = tf.reduce_mean(losses.binary_crossentropy(y_train, pre))
            # print(loss.numpy())
        # # with summary.as_default():
            # # tf.summary.scalar('loss', loss, i)
        # grad = tape.gradient(loss, att.variables)
        # optimizer.apply_gradients(grads_and_vars=zip(grad, att.variables)) 	
    # att.save('att.h5')
    # print('save ok!')
    # import sys
    # exit()
    # saver = tf.train.Saver(write_version=tf.train.SaverDef.V2)
    # saver.save(sess, "../data/18416/att_model.ckpt", global_step=0)
    # output_feature = att(input_feature.detach().numpy())
    # print(type(output_feature))
...
# def attention(hops, adj, feature_list, alpha=0.15):
    ...
    # # output_feature.numpy()
    # # output_feature.eval(session=sess)
    # print(output_feature.shape) 
    # # print(type(output_feature))
    # return output_feature

# def attention2(hops, adj, feature_list, Embedding, alpha=0.5):
    # input_feature = []
    # for i in range(adj.shape[0]):
        # hop = hops[i].int().item()
        # if hop == 0:
        ...
        # else:
            ...
            # allfeatures = []
            # for j in range(hop):
                # fea += (1-alpha)*feature_list[j][i].unsqueeze(0) + alpha*Embedding[i].unsqueeze(0)
            # fea = fea / hop
        # input_feature.append(fea)
    # input_feature = torch.cat(input_feature, dim=0)
    # return input_feature

# def attention1(hops, adj, feature_list, Embedding, out_dim, alpha=0.15):
    # input_feature = []
    # for i in range(adj.shape[0]):
        # hop = hops[i].int().item()
        # if hop == 0:
        ...
        # else:
            ...
            # for j in range(hop):
                ...
                # allfeatures.append(fea[0])
            # allf = fea_conver(torch.stack(allfeatures,0).detach().numpy().tolist())
            # # GAT = GraphAttentionLayer(allf.shape[1],out_dim)
            # # att = GAT(allf,torch.from_numpy(random.randint(1,2,size=(allf.shape[0],allf.shape[0])))) #全是1的矩阵
            # # M = sprase_rand(allf.shape[0], allf.shape[0], 0.1)
            # # M = sparse_mx_to_torch_sparse_tensor(M)
            # # M = normalize(torch.where(M>0,0,1) + torch.eye(allf.shape[0]))
            # # M = torch.from_numpy(M)
            # # att = GAT(allf,M)
        # input_feature.append(allf[hop-1])
    # input_feature = torch.cat((fea_conver(torch.stack(input_feature,0).detach().numpy().tolist()),Embedding),1)
    # return input_feature
